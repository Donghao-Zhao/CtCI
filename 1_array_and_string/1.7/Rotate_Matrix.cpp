    ...
    bool myString_1_7::rotateMatrix(int **myMatrix) {
        ...
        for (int i = 0; i != COLUMN / 2; i++) {
            for (int j = i; j != (COLUMN - 1 - i); j++) {
                ...
                *((int *) myMatrix + i * COLUMN + j) = *((int *) myMatrix + (END_INDEX - j) * COLUMN +
                                                         i);    // top <-- left
                *((int *) myMatrix + (END_INDEX - j) * COLUMN + i) = *((int *) myMatrix + COLUMN * (END_INDEX - i) +
                                                                       (END_INDEX - j)); // left <-- bot
                *((int *) myMatrix + COLUMN * (END_INDEX - i) + (END_INDEX - j)) = *((int *) myMatrix + COLUMN * j +
                                                                                     (END_INDEX -
                                                                                      i));    // bot <-- right
